...
async def start_command(client: Client, message: Message):
    ...
    if len(text)>7:
        ...
        k = await client.send_message(chat_id = message.from_user.id, text=f"gabisa di forward cuy yahaha😹")
...
async def not_joined(client: Client, message: Message):
    buttons = [
        [
            ...
            InlineKeyboardButton(text="Cʜᴀɴɴᴇʟ 3 🐷", url=client.invitelink2),
            InlineKeyboardButton(text="Cʜᴀɴɴᴇʟ 4 🐥", url=client.invitelink2),
        ]
    ]
    try:
        buttons.append([
